#!/bin/bash
echo -e "\e[32m[task1]\e[0m modifica file host"
...
echo -e "\e[32m[task2]\e[0m disabilitare selinux"
sudo setenforce 0
sudo sed -i 's/SELINUX=permissive\|SELINUX=enforcing/SELINUX=disabled/g' /etc/selinux/config

echo -e "\e[32m[task3]\e[0m disabilitare firewalld"
sudo systemctl disable firewalld --now
sudo systemctl stop firewalld

echo -e "\e[32m[task4]\e[0m disabilitare swap"
sudo swapoff -a
sudo sed -i '/ swap / s/^\(.*\)$/#\1/g' /etc/fstab

echo -e "\e[32m[task5]\e[0m networking kubernetes"
...
echo -e "\e[32m[task6]\e[0m Setup proxy and repos for yum"
...
echo -e "\e[32m[task7]\e[0m Docker installation"
...
echo -e "\e[32m[task8]\e[0m kubernetes installation"
...
echo -e "\e[32m[task9]\e[0m delete config file containerd"
sudo rm /etc/containerd/config.toml
sudo systemctl restart containerd

echo -e "\e[32m[task10]\e[0m install Helm"
curl -fsSL -o get_helm.sh https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3
chmod 700 get_helm.sh
./get_helm.sh

echo -e "\e[32m[task11]\e[0m kubeadm init master"
sudo kubeadm init --apiserver-advertise-address=172.16.16.100 --pod-network-cidr=10.244.0.0/16

echo -e "\e[32m[task12]\e[0m enable kubectl for user"
mkdir -p $HOME/.kube
